...
# source('pairwise.adonis2_hr.r')
#

# example:
# data(iris)
# pairwise.adonis(iris[,1:4],iris$Species, "bray")
#
...
pairwise.adonis <- function(x,factors, sim.method = 'euclidian', p.adjust.m ='holm')
{
  library(vegan)
  ...
  for(elem in 1:ncol(co)){
    if(sim.method %in% c("steinhaus","sorensen","ochiai","ruzicka","roberts","chisq")){
      x1 = dsvdis(x[factors %in% c(co[1,elem],co[2,elem]),],index = sim.method)
